...
def clean_text(text):
    ...
    return ''.join([w + " " for w in stripped])
...
def load_text():
    ...
    if clean:
        ...
        desc = [clean_text(d) for d in desc]
...
def plot_word_distributions(vocab, vocab_count):
    """Plot word distribution in headlines and description."""
    plt.plot([vocab_count[w] for w in vocab])
    plt.gca().set_xscale("log", nonposx='clip')
    plt.gca().set_yscale("log", nonposy='clip')
    title = 'word distribution in headlines and description'
    plt.title(title)
    plt.xlabel('rank')
    plt.ylabel('total appearances')
    plt.savefig(path.join(config.path_visual_reports, '{}.png'.format(title)))
...
def to_dense_vector(word2idx, corpus, description, bins=50):
    ...
    plt.savefig(path.join(config.path_visual_reports, '{}_distribution.png'.format(description)))
    return data


def inspect_vocab(vocab, vocab_count):
...
def main():
    ...
    inspect_vocab(vocab, vocab_count)  # summarize vocabulary
