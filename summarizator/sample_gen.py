...
def lpadd(x):
    ...
    assert MAXLEND >= 0
    if MAXLEND == 0:
        return [EOS_VOCAB_IDX]
    n = len(x)
    if n > MAXLEND:
        x = x[-MAXLEND:]
        n = MAXLEND
    return [EMPTY_VOCAB_IDX] * (MAXLEND - n) + x + [EOS_VOCAB_IDX]
...
def beamsearch(
    ...
    while live_k:
        ...
        zombie = [s[-1] == EOS_VOCAB_IDX or len(s) > maxsample for s in live_samples]
...
def keras_rnn_predict(samples, empty, model, batch_size):
    ...
    assert all(l[MAXLEND] == EOS_VOCAB_IDX for l in samples)
...
def gensamples(
    ...
    for s in skips:
        ...
        sample, score = beamsearch(
            ...
            start=fold_start,
            maxsample=MAXLEN,
            empty=EMPTY_VOCAB_IDX,
            nb_unknown_words=nb_unknown_words,
            vocab_size=vocab_size,
            avoid=avoid,
            **kwargs
        )
        assert all(s[MAXLEND] == EOS_VOCAB_IDX for s in sample)
