...
model.add(Embedding(vocab_size, embedding_size,
                    ...
                    name='embedding_1'))
model.add(SpatialDropout1D(p_emb, name='dropout_%d' % 0))
for i in range(1, rnn_layers+1):
    ...
    model.add(lstm)
    model.add(SpatialDropout1D(p_dense, name='dropout_%d' % (i + 1)))
...
def gensamples(skips=2, k=10, batch_size=batch_size, short=True, temperature=1., use_unk=True):
    ...
    for s in skips:
        ...
        print("-"*99)
        sample, score = beamsearch(predict=keras_rnn_predict, start=fold_start, k=k, temperature=temperature,
                                   use_unk=use_unk)
...
def flip_headline(x, nflips=None, model=None, debug=False):
    ...
    for b in range(batch_size):
        ...
        for input_idx in flips:
            ...
            if debug and b < debug:
                print('%s => %s' % (
                idx2word[x_out[b, input_idx]].encode('ascii', 'ignore'), idx2word[w].encode('ascii', 'ignore')), )
            x_out[b, input_idx] = w
        if debug and b < debug:
            print()
    return x_out
